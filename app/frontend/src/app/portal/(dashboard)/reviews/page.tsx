...
export default function PortalReviewsPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        const res = await fetch("/api/portal/my-reviews?per_page=30");
    ...
    <div className="grid gap-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Reviews you&apos;ve written for trainers. Write one from a trainer&apos;s profile page.
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-base">Submitted Reviews</CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              ...
              {loading ? (
                ...
                reviews.map((review) => (
                  <TableRow key={review.id}>
                    <TableCell className="font-medium">{review.trainer.user.name}</TableCell>
                    <TableCell>
